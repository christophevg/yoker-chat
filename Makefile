...
pre-publish: check ## Pre-publication checks (run before publishing)
	...
	VERSION_INIT=$$(grep '^__version__ = ' src/yoker_chat/__init__.py | cut -d'"' -f2); \
...
# - client: Run yoker-chat with local server
# - online-client: Run yoker-chat with production server
